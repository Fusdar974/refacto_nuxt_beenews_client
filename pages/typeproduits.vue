<template>
    <v-container>
            ...
            <v-col  >
                <v-table>
                    ...
                    >
                        ...
                        <td>
                            <v-checkbox v-model="type.proposablePot" @change="updateType(type)" color="pink"></v-checkbox>
                        </td>
                        <td>
                            <v-checkbox v-model="type.proposableSoum" @change="updateType(type)" color="pink"></v-checkbox>
                        </td>
                        <td v-if="type.nombreProduits >= 1">{{ type.nombreProduits }} produits en stock</td>
                        <td v-if="type.nombreProduits < 1">
                            <v-btn  color="pink" icon="mdi:mdi-delete" @click="supprimer(type._id)">
                            </v-btn>
                        </td>
                    </tr>
                    </tbody>
        ...
        <v-row  align="start">
            <v-col>
                <v-text-field type="number" v-model="nombreBnOfferts" label="valeur" :rules="[v => typeof v !== 'number' || 'Veuillez rentrer un nombre']"></v-text-field>
            </v-col>
            <v-col>
                <v-select v-model="selectedReciever"
                          :items="users"
            ...
            <v-col cols="auto">
                <v-btn color="primary"  @click="offrirBN()">Offrir Exceptionnellement</v-btn>
            </v-col>
        </v-row>
        <v-row justify="space-between" align="start">
            <v-col cols="auto">
        ...
        <v-row  align="start">
            <v-col >
                <v-text-field type="number" v-model="valeurBn" @change="setBnState"></v-text-field>
            </v-col>
        </v-row>
    </v-container>
</template>
...
async function updateType(type: TypeProduitInterface) {
    if (type.nom !=='') {
        await Fetch.requete({ url: `/typeproduits/${type._id}`, method: 'PUT', data: { typeproduit: type } });
    }
}
function ajouter() {
    ...
    Fetch.requete({ url: `/typeproduits/create`, method: 'POST', data: { typeproduit: emptyType }}, (res: { typeproduit: TypeProduitInterface; }) => {
        types.value.push(res.typeproduit);
    });
}
function  supprimer(typeId: string) {
    if(typeId) {
...
function offrirBN() {
    if (selectedReciever.value && nombreBnOfferts.value > 0) {
        Fetch.requete({
                ...
                method: 'POST',
                data: { pointOffert: nombreBnOfferts.value, commentaires: 'Ajout exceptionnel' }
            },
            () => {
...
const getTypes = ()  =>  {
    Fetch.requete({ url: '/typeproduits'}, (resultat: Array<TypeProduitInterface>) => {
        types.value = resultat;
    })
}
const getUsers = ()  =>  {
    Fetch.requete({ url: '/users',data: { page: 1, nombre: 1000 }}, (resultat: { documents: any; }) => {
        users.value = resultat.documents;
        selectedReciever.value = resultat.documents[0];
    })
}
const getBnState = () => {
    Fetch.requete({ url: '/parametre/valeurBN', method: 'GET' }, (resultat:  any) => {
